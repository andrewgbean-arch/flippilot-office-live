...
describe("loading the stock when one stored car is odd", () => {
  ...
  it("a car an older version saved with the invented scores shows none of them, and an edit sends only the edited field", async () => {
    stored = [oldCar("old")];
    await open();
    ...
    // Saves send only what changed, so the server's old copy keeps its leftovers
    // (nothing on the backend reads them) until that car is replaced whole. What
    // matters is that they never come back onto the screen or into what is sent.
    const sent = calls.filter(c => c.method === "PUT");
    expect(sent).toHaveLength(1);
    expect(sent[0]!.body.changes).toEqual([{ id: "old", set: { notes: "Two keys" } }]);
