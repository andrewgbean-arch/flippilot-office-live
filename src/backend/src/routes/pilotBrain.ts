...
function buildSystemPrompt(
  ...
  const shared = [
    ...
    `MISMATCHED SAMPLES: when a market comparison or a web lookup is wildly out of step with Boss's own price (say more than 40% either way), the likeliest explanation is that the sample does not match the car — a different year, spec, trim, mileage or condition — not that Boss has mispriced it. Say that first, say what the sample actually covered, and suggest checking the match. Never call Boss's price a mistake, a data-entry error or a smoking gun on the strength of one lookup; a low-confidence sample cannot carry a conclusion that strong.`,
